...
async function updateExistingPhraseLevels() {
  try {
    ...
    if (phraseLevels.length === 0) {
      console.log('❌ フレーズレベルが見つかりません。先にsetup-phrase-levels.tsを実行してください。')
      return
    }
    ...
    for (const phrase of phrases) {
      ...
      if (!expectedLevelId) {
        console.log(`⚠️  フレーズ ${phrase.id}: 正解数 ${correctAnswers} に対するレベルが見つかりません`)
        continue
      }
      ...
      if (phrase.phraseLevelId !== expectedLevelId) {
        ...
        const oldLevel = phraseLevels.find(l => l.id === phrase.phraseLevelId)
