...
export const useAuthGuard = (redirectPath = '/', requireUserSetup = true) => {
  ...
  useEffect(() => {
    ...
    if (requireUserSetup && userSettings === null && !hasRedirectedToSettings.current) {
      ...
      timeoutRef.current = setTimeout(() => {
        ...
        if (userSettings === null && !userSettingsLoading && !hasRedirectedToSettings.current) {
          const latestCachedStatus = userSetupCache.getUserSetupComplete(user.id)
