...
export function useSituations(): UseSituationsReturn {
	...
	const fetchSituations = useCallback(async () => {
		try {
			setIsLoading(true);
			setError(null);
			const data = await api.get<{ situations: SituationResponse[] }>(
				"/api/situations",
			);
			setSituations(data.situations);
		} catch (err) {
			setError(
				err instanceof Error ? err.message : "Failed to fetch situations",
			);
			setSituations([]);
		} finally {
			setIsLoading(false);
	...
	const addSituation = useCallback(async (name: string) => {
		try {
			const newSituation = await api.post<SituationResponse>(
				"/api/situations",
				{ name },
			);
