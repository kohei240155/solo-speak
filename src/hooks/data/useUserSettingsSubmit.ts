    ...
    try {
      ...
      try {
        ...
        currentLanguageSettings = {
          ...
          defaultLearningLanguageId:
            currentSettings.defaultLearningLanguageId || "",
        };
      } catch (error) {
      ...
      if (
        isFirstTimeSetup &&
        user &&
        (!finalData.iconUrl || finalData.iconUrl.trim() === "")
      ) {
        ...
        if (
          ...
          try {
            ...
            const googleUploadResult = (await api.post(
              "/api/user/icon/google",
              { googleAvatarUrl },
            )) as { iconUrl: string };
      ...
      if (
        finalData.iconUrl === "" &&
        existingIconUrl &&
        existingIconUrl.trim() !== ""
      ) {
        // Supabase StorageのURLかどうかをチェック
        const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL || "";
        const isSupabaseUrl =
          existingIconUrl.includes(supabaseUrl) &&
          ...
          api
            ...
            .then((response) => {
              ...
              const selectedLanguage = languages.find(
                (lang) => lang.id === data.nativeLanguageId,
              );
              if (selectedLanguage?.code) {
                window.dispatchEvent(
                  new CustomEvent("nativeLanguageChanged", {
      ...
      if (error instanceof ApiError) {
        ...
        if (error.status === 404 && error.message.includes("User not found")) {
          setError(
            "ユーザー設定の初期化に失敗しました。ページを再読み込みして再試行してください。",
          );
        } else {
        ...
        setError(
          `ユーザー設定の保存に失敗しました: ${error instanceof Error ? error.message : "Unknown error"}`,
        );
      }
    } finally {
      setSubmitting(false);
