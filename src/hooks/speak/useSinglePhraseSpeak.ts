...
import { useSpeakPhraseById, useInfinitePhrases } from "@/hooks/api";
...
export function useSinglePhraseSpeak({
	phraseId,
	learningLanguage,
	sendPendingCount,
	...
	const [singlePhraseCountDisabled, setSinglePhraseCountDisabled] =
		useState(false);
	...
	const handleFinish = useCallback(async () => {
		...
		try {
			...
			if (singlePhrase && singlePhrasePendingCount > 0) {
				const success = await sendPendingCount(
					singlePhrase.id,
