...
export function useUserSettingsData(userId: string | null) {
	const userSettingsKey = userId
		? ([`/api/user/settings`, userId] as const)
		: null;

	const { data, error, isLoading, mutate } = useSWR(
		userSettingsKey,
		async ([url]) => {
			return await fetcher<UserSettingsResponse>(url, {
...
export function useDashboardData(language?: string) {
	...
	const dashboardKey =
		user && language
			? ([`/api/dashboard?language=${language}`, user.id] as const)
			: null;

	const { data, error, isLoading, mutate } = useSWR(
		dashboardKey,
		([url]) => fetcher<DashboardData>(url),
...
export function usePhrase(phraseId?: string) {
	...
	// ユーザー切り替え時のキャッシュ衝突を避けるためにuser.idをキーに含める
	const phraseKey =
		user && phraseId ? ([`/api/phrase/${phraseId}`, user.id] as const) : null;

	const { data, error, isLoading, mutate } = useSWR(
		phraseKey,
		([url]) => fetcher<PhraseDetailResponse>(url),
		SWR_CONFIGS.MEDIUM_CACHE,
	);
...
export function useSpeakPhrase(language?: string) {
	...
	const speakKey =
		user && language
			? ([`/api/phrase/speak?language=${language}`, user.id] as const)
			: null;

	const { data, error, isLoading, mutate } = useSWR(
		speakKey,
		([url]) => fetcher<SpeakPhraseResponse>(url),
		{
...
export function useRemainingGenerations() {
	...
	// ユーザー切り替え時のキャッシュ衝突を避けるためにuser.idをキーに含める
	const generationsKey = user
		? (["/api/phrase/remaining", user.id] as const)
		: null;

	const { data, error, isLoading, mutate } =
		useSWR<RemainingGenerationsResponse>(
			generationsKey,
			([url]) =>
				fetcher<RemainingGenerationsResponse>(url, { showErrorToast: false }),
			SWR_CONFIGS.MEDIUM_CACHE,
		);
...
export function usePhraseStreakRanking(language?: string) {
	...
	const { data, error, isLoading, mutate } = useSWR(
		rankingKey,
		([url]) =>
			fetcher<import("@/types/ranking").PhraseStreakRankingResponseData>(url),
		SWR_CONFIGS.SHORT_CACHE,
	);
...
export function useSpeakStreakRanking(language?: string) {
	...
	const { data, error, isLoading, mutate } = useSWR(
		rankingKey,
		([url]) =>
			fetcher<import("@/types/ranking").SpeakStreakRankingResponseData>(url),
		SWR_CONFIGS.SHORT_CACHE,
	);
...
export function useQuizStreakRanking(language?: string) {
	...
	const { data, error, isLoading, mutate } = useSWR(
		rankingKey,
		([url]) =>
			fetcher<import("@/types/ranking").QuizStreakRankingResponseData>(url),
		SWR_CONFIGS.SHORT_CACHE,
	);
