...
export function useQuizPhrase(): UseQuizPhraseReturn {
	...
	const fetchQuizSession = useCallback(
				...
				if (data.success && data.phrases && data.phrases.length > 0) {
					const newSession: QuizSession = {
						...
						availablePhraseCount:
							data.availablePhraseCount || data.phrases.length,
					};
	...
	const handleNext = useCallback(() => {
		...
		setSession((prevSession) => {
			if (
				!prevSession ||
				prevSession.currentIndex >= prevSession.totalCount - 1
			) {
				return prevSession;
			}
			return {
	...
	const handleSpeakCount = useCallback(
		async (phraseId: string, count: number) => {
			try {
				// 指定された回数分音読回数を加算
