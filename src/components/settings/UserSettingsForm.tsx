...
export default function UserSettingsForm({
  ...
  } = useUserSettingsSubmit(setError, setIsUserSetupComplete);
  ...
  const isSameLanguage =
    watchNativeLanguageId &&
    watchDefaultLearningLanguageId &&
    watchNativeLanguageId === watchDefaultLearningLanguageId;
      ...
      <form onSubmit={handleSubmit(onSubmitFromHook)} className="space-y-6">
        ...
        <div>
          ...
          {errors.nativeLanguageId && (
            <p className="mt-1 text-sm text-red-600">
              {errors.nativeLanguageId.message}
            </p>
          )}
        </div>
        ...
        <div>
          <label
            htmlFor="defaultLearningLanguageId"
            className="block text-gray-700 mb-2 text-base md:text-lg font-bold"
          >
          ...
          <div className="relative">
            ...
            >
              ...
              {languages
                .filter((lang) => lang.id !== watchNativeLanguageId)
                .map((lang) => (
                  <option key={lang.id} value={lang.id}>
                    {lang.name}
                  </option>
          ...
          {errors.defaultLearningLanguageId && (
            <p className="mt-1 text-sm text-red-600">
              {errors.defaultLearningLanguageId.message}
            </p>
          )}
          {isSameLanguage && (
            <p className="mt-1 text-sm text-red-600">
              {t("settings.validation.sameLanguageError")}
            </p>
          )}
        </div>
        ...
        <div className="pt-4">
          <button
            ...
            style={{
              backgroundColor:
                actualSubmitting || !!isSameLanguage ? "#9CA3AF" : "#616161",
            }}
            onMouseEnter={(e) => {
