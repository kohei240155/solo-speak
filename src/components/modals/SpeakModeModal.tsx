...
export default function SpeakModeModal({
	isOpen,
	onClose,
	onStart,
	languages,
	...
	const handleStart = async (selectedLanguage: string) => {
		...
		try {
			...
			const countResult = await getSpeakPhraseCount(selectedLanguage, {
				excludeIfSpeakCountGTE: excludeThreshold
					? parseInt(excludeThreshold, 10)
					: undefined,
			...
			if ("error" in countResult) {
				// エラーの場合
				const errorMessage =
					countResult.error || t("speak.messages.phraseCountFetchFailed");
				toast.error(errorMessage);
				return;
			}
		...
		configItems: [
			{
				...
				onChange: (value: string | boolean) =>
					setExcludeThreshold(value as string),
			},
			{
