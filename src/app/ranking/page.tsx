...
export default function RankingPage() {
	...
	const handleShareStreak = async () => {
		...
		const userRankData = rankingData.find(
			(rankUser) => rankUser.userId === user?.id,
		);
		if (userRankData) {
		...
		await shareStreak(
			...
			userCount,
			activeRankingType,
			activeTab as "Daily" | "Weekly" | "Total" | "Streak",
		);
	};
					...
					<div className="overflow-hidden px-2 sm:px-4 md:px-8">
						...
						{!isLoading && currentUser && currentUser.rank > 10 && (
							<div className="mt-6 pt-4 border-t border-gray-200">
								...
								<div className="grid grid-cols-[40px_1fr_60px] sm:grid-cols-[50px_1fr_70px] gap-2 py-2 sm:py-3 px-1 sm:px-2 rounded-lg bg-blue-50 border border-blue-200">
									{/* 順位 */}
									<div className="flex items-center justify-center">
										<span className="text-gray-600 font-medium text-sm sm:text-base">
											{currentUser.rank}
