...
export async function GET(request: NextRequest) {
	try {
		...
		const excludeTodayQuizzed =
			searchParams.get("excludeTodayQuizzed") === "true";
		...
		if (!mode || (mode !== "normal" && mode !== "random")) {
			return NextResponse.json(
				{
					error:
						'Mode parameter is required and must be either "normal" or "random"',
				},
				{ status: 400 },
			);
		}
		...
		// 音読回数フィルターを取得
		const speakCountMinimum = speakCountFilter
			? parseInt(speakCountFilter, 10)
			: null;
